...
**What was implemented:**
- Keep last 5 tagged image versions via `actions/delete-package-versions@v5` (auto-cleanup untagged only)
- Persistent rollback state saved to `/var/lib/deploy-rollback/` (survives reboots)
- Saves actual image digests (not container refs) for reliable rollback
- Auto-rollback on health check failure: reverts git, pulls saved image digests, restarts containers
...
# View saved rollback state
cat /var/lib/deploy-rollback/commit.txt
cat /var/lib/deploy-rollback/backend.txt
cat /var/lib/deploy-rollback/frontend.txt

# Execute manual rollback
PREV_COMMIT=$(cat /var/lib/deploy-rollback/commit.txt)
git reset --hard $PREV_COMMIT

# Pull saved image digests
docker pull "$(cat /var/lib/deploy-rollback/backend.txt)"
docker pull "$(cat /var/lib/deploy-rollback/frontend.txt)"
...
# Validate saved images exist in registry
docker manifest inspect $(cat /var/lib/deploy-rollback/backend.txt)
```
