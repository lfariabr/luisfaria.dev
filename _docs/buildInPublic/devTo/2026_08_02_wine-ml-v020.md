...
I closed that gap in v0.2.1 with a separate regression lineage contract. It reproduces
both final-estimator protocols from the public UCI data: the submitted 6,497-row result and
the 5,320-row production adaptation. Training now reads the serving estimator, split, row
policy, expected metrics, source commit, and hashes from `mln601-a1-derived-v1` instead of
constants spread across Python and tests.
...
The production regressor did not move. Its artifact hash, metrics, and golden `5.065`
prediction remained unchanged. What changed was the strength and honesty of the claim I
could make about it.
...
This changed provenance from prose into two executable guarantees. Training fails if the
dataset identity, row policy, feature order, metrics, or classification confusion matrix
diverges from the evidence declared for either model.
...
Module 9 was about K-means and the broader clustering family. Sommelier API v0.2.1 does
...
Sommelier API `v0.2.1` was released only after the following state agreed:
